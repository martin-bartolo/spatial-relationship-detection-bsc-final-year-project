...
for key, value in dict.items():
    ...
    for pred in value:
        if((vrd_preps[pred["predicate"]] in ["above", "on", "next to", "under", "in the front of", "behind", "near", "in", "below", "along", "across", "outside of"])
           and vrd_objects[pred["object"]["category"]] in englishObjectList
           and vrd_objects[pred["subject"]["category"]] in englishObjectList):
            preds.append(pred)
    if preds:
...
file = open('./features_vrd_filteredobjects.json', 'w+')
