...
#generate features and relations for unlabelled video examples
with open("features_unlabelledvideos.json", mode="r", encoding="utf-8") as f_uv:
    dict_uv = json.load(f_uv)
...
#add additional examples to training set
x_train = np.append(x_train, fullfeats, axis = 0)
y_train = np.append(y_train, rels, axis = 0)
...
#save the scalar
filename = './models/scalar_partfeatures_withlanguage_glove_WITHADDEDEXAMPLES.sav'
...
#save the model
filename = './models/multilabel_model_partfeatures_withlanguage_glove_WITHADDEDEXAMPLES.sav'
