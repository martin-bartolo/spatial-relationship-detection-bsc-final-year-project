...
for d in dict.values():
    ...
    feats.append([d['features']["AreaObj1_Norm_wt_Union"],
             ...
             d['features']["EuclDist_Norm_wt_UnionBB"],
...
rels = np.array(rels)
object1s = np.array(object1s)
object2s = np.array(object2s)

#binarize relationships
mlb = MultiLabelBinarizer()
rels = mlb.fit_transform(rels)
object1s = mlb.fit_transform(object1s)
object2s = mlb.fit_transform(object2s)

#split into training and testing
#iterative
np.random.seed(3)
x_train, y_train, x_test, y_test = iterative_train_test_split(feats, rels, test_size=0.2)
...
#save the scalar
filename = './models/scalar_partfeatures.sav'
pickle.dump(scaler, open(filename, 'wb'))

#create MLPClassifier instance
classifier = MLPClassifier(activation='relu', solver='adam', random_state=0, hidden_layer_sizes=(50,50,50), beta_1=0.92, beta_2=0.999)
...
#distributions = {'beta_1': [0.86,0.87,0.88,0.89,0.9,0.91,0.92,0.93,0.94,0.95], 'beta_2': [0.995,0.996,0.997,0.998,0.999,1]}
...
#save the model
filename = './models/multilabel_model_partfeatures.sav'
